...
pub const LOSS_SCORE: i32 = -1_000_000;

const PVS_START_DEPTH: u32 = 4;

/// Searches a game state using iterative deepening Minimax with alpha–beta,
/// transposition table, and Principal Variation Search (PVS).
///
/// The search is performed repeatedly with increasing depth limits,
/// starting from depth 1 and continuing until either:
/// - the maximum search depth specified in `base_config` is reached, or
/// - an optional time limit (`time_ms` in `base_config`) is exceeded.
///
...
///
/// Principal Variation Search is applied on top of alpha–beta at nodes
/// where the remaining search depth is at least 4. At those nodes, the
/// first child is searched with a full window and the remaining children
/// are searched with a null window, with a full re‑search only when
/// necessary. At shallower remaining depths (< 4), the search falls back
/// to plain alpha–beta, since PVS is not beneficial there.
///
/// # Parameters
///
/// * `state` - Root position to evaluate.
/// * `base_config` - Search configuration containing the maximum search
///   depth, optional time limit, and evaluation settings.
///
/// # Returns
///
/// The result of the deepest completed search (within the depth and/or
/// time limits), including the selected move and its evaluation score.
...
/// Searches the game tree using Minimax with alpha–beta pruning,
/// a transposition table, and Principal Variation Search (PVS).
///
...
///
/// On top of plain alpha–beta, the search uses Principal Variation Search
/// at nodes where the remaining search depth is at least 4. At such nodes,
/// the first child is searched with a full alpha–beta window, and the
/// remaining children are searched with a null window, with a full re‑search
/// only when necessary. At shallower remaining depths (< 4), the search
/// falls back to plain alpha–beta, since PVS is not beneficial there.
///
/// # Parameters
///
/// * `state` - Root position to evaluate.
/// * `config` - Search configuration, including depth limit and evaluation settings.
...
mod tests {
    ...
    fn test_iterative_deepening_with_time_per_move_not_exceeded() {
        ...
        let start = Instant::now();
        let _result = minimax(&state, &config);
